...
def _fc(x, n_classes, reuse):
    '''
    x: N x C 
    '''
    input_shape = x.get_shape().as_list()[1]
    with tf.variable_scope("fc_shareweight", reuse = reuse) as scope:
        W = tf.get_variable(name="weights", shape=[input_shape, n_classes],dtype=tf.float64, initializer=tf.contrib.layers.xavier_initializer())
...
def eval():
    with tf.Graph().as_default():
        ...
        np.savetxt(os.path.join(config.log_folder, config.config_name,'./eval.pred.'+str(gstep)+'.txt') , test_pred, fmt='%d')
        np.savetxt(os.path.join(config.log_folder, config.config_name,'./eval.score.'+str(gstep)+'.txt'), test_score, fmt="%.8f")
