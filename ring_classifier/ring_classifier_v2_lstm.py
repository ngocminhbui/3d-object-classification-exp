...
class MultiviewDataset():
    LIST_RING = [
        ...
        [7,24,3,11,19,25,23,15 ],
        [1,24,5,13,21,25,17,9 ],
        [0, 24, 4, 12, 20, 25, 16, 8],
        [2, 24, 6, 14, 22, 25, 18, 10]
    ]
    ...
    def __init__(self, x_path, y_path, batch_size = 32, shuffle = None):
        ...
        tmp = [np.expand_dims(makeRing(X, self.LIST_RING[i]),0) for i in range(len(self.LIST_RING))]
        ...
        #fix dims
        self.X = self.X.reshape(-1, 8, 2048) #  (Nx7), 8 , 2048
...
def inference(x, y, is_training, n_classes, keep_prob):
    ...
    x = tf.stack(view_features,0) # V x N x 128 
    x = tf.transpose(view_features,[1,0,2]) # N x V x 128

    rnn_cell = tf.nn.rnn_cell.LSTMCell(n_classes, reuse= tf.AUTO_REUSE) 
    initial_state = rnn_cell.zero_state(tf.shape(x)[0], dtype=tf.float64)
    outputs, state  = tf.nn.dynamic_rnn(rnn_cell, x, initial_state = initial_state, dtype = tf.float64)

    x = state.c # N x n_classes
    #print x.h
    #x = tf.reduce_mean(x,1) # N x n_classes
    return x
...
def eval():
    with tf.Graph().as_default():
        #placeholders
        x_place_holder = tf.placeholder(tf.float64, shape=(None, 8, 2048), name="input_data")
...
def train():
    with tf.Graph().as_default():
        ...
        last_increase_epoch = 0
        for epoch in xrange(config.max_epoch):
            sys.stdout.flush()
            if epoch-last_increase_epoch >= 200:
